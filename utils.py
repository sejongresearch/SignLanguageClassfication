...
def calculate_mAP(det_boxes, det_labels, det_scores, true_boxes, true_labels):
    ...
    for c in range(1, n_classes):
        ...
        cumul_recall = cumul_true_positives / cumul_true_positives.shape[0]  # (n_class_detections)


        # #####################################################################################################

        import matplotlib.pyplot as plt
        plt.plot( cumul_recall.tolist(), cumul_precision.tolist(), '.' )
        plt.savefig('prec-recall.jpg')
        ...
        for i, t in enumerate(recall_thresholds):
            ...
            if recalls_above_t.any():                
                ...
                precisions[i] = cumul_precision[recalls_above_t][-1]
            else:
                precisions[i] = 0.
            ...
            # if recalls_above_t.any():
            #     # precisions[i] = cumul_precision[recalls_above_t].max()
            #     precisions[i] = cumul_precision[recalls_above_t].min()
            # else:
            #     precisions[i] = 0.
...
def resize(image, boxes, dims=(512, 640), return_percent_coords=True):
    """
    Resize image. For the SSD_512x640, resize to (512, 640).
